...
class UnifiedTwitchBot {
    constructor() {
        // Настройки подключения с поддержкой порта из переменных окружения Render (PORT)
    ...
    start() {
        ...
        // Подключение к Twitch и веб-сервер для Render
        this.Connect();
    ...
    // Добавлен HTTP-сервер для прохождения проверки порта на Render (Web Service требует открытый порт)
    StartHttpServer() {
        ...
        server.listen(port, '0.0.0.0', () => {
            console.log(`🌐 HTTP-сервер запущен и слушает порт ${port} для Render.`);
        });
    ...
    HandleCasinoCommand(user, message) {
        ...
        // --- КОМАНДА ВЫВОДА КРЫШЕК (НА КАРТУ ИЛИ НАЛИЧКОЙ) ---
        if (lowerText.startsWith("!вывод")) {
            let parts = text.split(' ');
            if (parts.length < 3) {
                this.Send(this.channelName, "💳 @" + user + ", используйте формат: !вывод карта [сумма] или !вывод нал [сумма]");
                return;
            }
            let method = parts[1].toLowerCase();
            let amount = parseInt(parts[2]);

            if (isNaN(amount) || amount <= 0) {
                this.Send(this.channelName, "❌ Укажите корректную сумму для вывода!");
                return;
            }

            if (method === "карта") {
                if (profile.bankCardBalance < amount) {
                    this.Send(this.channelName, "❌ На балансе карты недостаточно крышек для вывода!");
                    return;
                }
                profile.bankCardBalance -= amount;
                this.coins.set(user, profile.bankCardBalance);
                profile.balance += amount; // переводим в наличные батоны/средства
                this.saveDataCasino();
                this.Send(this.channelName, "💳 @" + user + ", успешно выведено " + amount + " крышек с банковской карты в наличные!");
                return;
            } else if (method === "нал" || method === "наличные") {
                if (profile.balance < amount) {
                    this.Send(this.channelName, "❌ У вас недостаточно наличных для этой операции!");
                    return;
                }
                ...
                this.coins.set(user, profile.bankCardBalance);
                this.saveDataCasino();
                this.Send(this.channelName, "💵 @" + user + ", наличные в размере " + amount + " успешно зачислены на баланс карты!");
                return;
            } else {
                this.Send(this.channelName, "❌ Неизвестный метод вывода. Используйте: !вывод карта [сумма] или !вывод нал [сумма]");
                return;
            }
        }

        // --- ВОССТАНОВЛЕННЫЙ МАГАЗИН И ВСЕ 30 БОНУСОВ ---
        if (lowerText === "!магазин" || lowerText === "!shop") {
            this.Send(this.channelName, "🛒 МАГАЗИН: 1. !купить дом [эконом/стандарт/элитный/роскошный/президентский] | 2. !купить работу [название] | 3. !купить бонус [вип/удача/щит/двойной/...] | 4. !работа");
            return;
        }
        ...
        if (lowerText.startsWith("!купить")) {
            let parts = text.split(' ');
            if (parts.length < 3) {
                this.Send(this.channelName, "🛒 Используйте: !купить дом [тип], !купить работу [название] или !купить бонус [название]");
                return;
            }
            let category = parts[1].toLowerCase();
            let itemName = parts.slice(2).join(" ").toLowerCase();

            if (category === "дом") {
                if (!this.houseCosts[itemName]) {
                    this.Send(this.channelName, "❌ Неизвестный тип дома. Доступные: эконом, стандарт, элитный, роскошный, президентский.");
                    return;
                }
                let cost = this.houseCosts[itemName];
                if (profile.balance < cost) {
                    this.Send(this.channelName, "❌ Недостаточно средств для покупки дома '" + itemName + "' (Стоит: " + cost + ")");
                    return;
                }
                profile.balance -= cost;
                profile.houseType = itemName;
                this.saveDataCasino();
                this.Send(this.channelName, "🏠 Поздравляем! @" + user + " успешно обрёл(а) дом класса '" + itemName + "'!");
                return;
            }

            if (category === "работа") {
                if (!this.fixedJobsSalary[itemName]) {
                    this.Send(this.channelName, "❌ Такой профессии нет. Проверьте список доступных работ.");
                    return;
                }
                profile.job = itemName;
                this.saveDataCasino();
                this.Send(this.channelName, "👷 @" + user + ", вы успешно устроились на работу: " + itemName + "!");
                return;
            }

            if (category === "бонус") {
                let bonusCost = 1000;
                if (profile.balance < bonusCost) {
                    this.Send(this.channelName, "❌ Стоимость покупки бонуса: " + bonusCost + " батонов.");
                    return;
                }
                profile.balance -= bonusCost;

                // Активация любого из 30 бонусов
                if (itemName.includes("вип")) { this.vipBonus.set(user, true); }
                else if (itemName.includes("удача")) { this.luckBonus.set(user, true); }
                else if (itemName.includes("щит")) { this.shieldBonus.set(user, true); }
                else if (itemName.includes("двойной")) { this.doubleBonus.set(user, true); }
                else if (itemName.includes("фриспин")) { this.freeSpin.set(user, true); }
                else if (itemName.includes("мегащит")) { this.megaShieldBonus.set(user, true); }
                else if (itemName.includes("джекпот")) { this.jackpotBonus.set(user, true); }
                else if (itemName.includes("тройной")) { this.tripleBonus.set(user, true); }
                else if (itemName.includes("супер-удача") || itemName.includes("супер удача")) { this.superLuckBonus.set(user, true); }
                else if (itemName.includes("магнит")) { this.magnetBonus.set(user, true); }
                else if (itemName.includes("хил")) { this.healBonus.set(user, true); }
                else if (itemName.includes("ультра-двойной") || itemName.includes("ультра двойной")) { this.ultraDoubleBonus.set(user, true); }
                else if (itemName.includes("гигащит")) { this.gigaShieldBonus.set(user, true); }
                else if (itemName.includes("крысиный король")) { this.ratKingBonus.set(user, true); }
                else if (itemName.includes("золотой батон")) { this.goldenBatonBonus.set(user, true); }
                else if (itemName.includes("сейф-долг")) { this.safeDebtBonus.set(user, true); }
                else if (itemName.includes("машина времени")) { this.timeWarpBonus.set(user, true); }
                else if (itemName.includes("омни-спин")) { this.omniSpinBonus.set(user, true); }
                else if (itemName.includes("теневой спин")) { this.shadowSpinBonus.set(user, true); }
                else if (itemName.includes("кибер-крыса")) { this.cyberRatBonus.set(user, true); }
                else if (itemName.includes("крыша мафии")) { this.mafiaCoverBonus.set(user, true); }
                else if (itemName.includes("ядерный спин")) { this.nuclearSpinBonus.set(user, true); }
                else if (itemName.includes("алхимик")) { this.alchemistBonus.set(user, true); }
                else if (itemName.includes("фантомная победа")) { this.phantomWinBonus.set(user, true); }
                else if (itemName.includes("королевский батон")) { this.royalBatonBonus.set(user, true); }
                else if (itemName.includes("титановый щит")) { this.titanShieldBonus.set(user, true); }
                else if (itemName.includes("божественная удача")) { this.godLuckBonus.set(user, true); }
                else if (itemName.includes("ключ матрицы")) { this.matrixKeyBonus.set(user, true); }
                else if (itemName.includes("синдикат")) { this.syndicateBonus.set(user, true); }
                else if (itemName.includes("абсолютный король")) { this.absoluteKingBonus.set(user, true); }
                else {
                    this.vipBonus.set(user, true);
                }

                this.saveDataCasino();
                this.Send(this.channelName, "🎁 Поздравляем, @" + user + ", вы приобрели бонус: " + itemName + "!");
                return;
            }
        }

        if (lowerText.startsWith("!слоты") || lowerText.startsWith("!казино")) {
            if (!this.isCasinoOpen) {
                this.Send(this.channelName, "⛔ Казино сейчас закрыто! Работает с 15:00 до 05:00.");
                return;
            }
            let parts = text.split(' ');
            let bet = parseInt(parts[1]);
            if (isNaN(bet) || bet <= 0) {
                this.Send(this.channelName, "🎰 @" + user + ", укажите корректную ставку: !слоты [сумма]");
                return;
            }
            if (profile.balance < bet) {
                this.Send(this.channelName, "❌ У вас недостаточно наличных батонов для такой ставки!");
                return;
            }

            profile.balance -= bet;
            let s1 = this.slots[Math.floor(Math.random() * this.slots.length)];
            let s2 = this.slots[Math.floor(Math.random() * this.slots.length)];
            let s3 = this.slots[Math.floor(Math.random() * this.slots.length)];

            if (s1 === s2 && s2 === s3) {
                let win = bet * 10;
                profile.balance += win;
                this.Send(this.channelName, "🎰 | " + s1 + " | " + s2 + " | " + s3 + " | JACKPOT! 🎉 @" + user + " выиграл(а) " + win + " батонов!");
            } else if (s1 === s2 || s2 === s3 || s1 === s3) {
                let win = bet * 2;
                profile.balance += win;
                this.Send(this.channelName, "🎰 | " + s1 + " | " + s2 + " | " + s3 + " | Победа! @" + user + " выиграл(а) " + win + " батонов.");
            } else {
                this.Send(this.channelName, "🎰 | " + s1 + " | " + s2 + " | " + s3 + " | Проигрыш. @" + user + ", повезёт в следующий раз!");
            }
            this.saveDataCasino();
            return;
        }

        if (lowerText === "!работать" || lowerText === "!работа") {
            if (profile.job === "Безработный") {
                this.Send(this.channelName, "👷 @" + user + ", вы безработный! Устройтесь на работу через !магазин.");
                return;
            }
            let salary = this.fixedJobsSalary[profile.job.toLowerCase()] || 200;
            profile.balance += salary;
            this.saveDataCasino();
            this.Send(this.channelName, "💼 @" + user + " отработал(а) смену на должности '" + profile.job + "' и заработал(а) " + salary + " батонов!");
            return;
        }
    }
