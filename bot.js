...
class UnifiedTwitchBot {
    ...
    OnApplicationQuit() {
        ...
        this.SaveUsersGreet();
        this.SaveDataFamily();
        this.loadDataCasino ? this.saveDataCasino() : null;
    ...
    ParseAndHandleMessage(line) {
        ...
        if (channelStart >= 0) {
            ...
            if (channelEnd > channelStart) {
                currentChannel = line.Substring ? line.Substring(channelStart + 1, channelEnd - channelStart - 1).toLowerCase() : line.substring(channelStart + 1, channelEnd).toLowerCase();
            }
        }

        let userIndex = line.indexOf('!');
        if (userIndex <= 1) return;
        let user = line.substring(1, userIndex - 1).toLowerCase().Trim ? line.substring(1, userIndex - 1).toLowerCase().trim() : line.substring(1, userIndex - 1).toLowerCase();
    ...
    InitQuestions() {
        this.questions = [
            ...
            { question: "Животное которое меняет цвет?", "хамелеон" },
            { question: "Самая большая птица?", "страус" },
            { question: "Дом пчёл?", "улей" },
            { question: "Первый человек на Луне?", "нил армстронг" },
            { question: "Год окончания Второй мировой войны?", "1945" },
            { question: "Кто построил пирамиды?", "египтяне" },
            { question: "Кто открыл Америку?", "христофор колумб" },
            { question: "Страна фараонов?", "египет" },
            { question: "Автор Войны и мира?", "толстой" },
            { question: "Первый президент США?", "джордж вашингтон" },
            { question: "Главный герой Матрицы?", "нео" },
            { question: "Злодей из Звёздных войн?", "дарт вейдер" },
            { question: "Зелёный герой Marvel?", "халк" },
            { question: "Герой с молотом Marvel?", "тор" },
            { question: "Герой с паутиной?", "человек паук" },
            { question: "Герой из Готэма?", "бетмен" },
            { question: "Школа Гарри Поттера?", "хогвартс" },
            { question: "Друг Шрека?", "осёл" },
            { question: "Сколько струн у гитары?", "6" },
            { question: "Сколько клавиш у пианино?", "88" },
            { question: "Человек который поёт?", "певец" },
            { question: "Инструмент в рок-музыке?", "гитара" },
            { question: "Музыкальный стиль с быстрым текстом?", "рэп" },
            { question: "Что такое CPU?", "процессор" },
            { question: "Что такое GPU?", "видеокарта" },
            { question: "Система Google для телефонов?", "android" },
            { question: "Магазин игр Valve?", "steam" },
            { question: "Компания создавшая PlayStation?", "sony" },
            { question: "Устройство для управления курсором?", "мышь" },
            { question: "Защита от вирусов?", "антивирус" },
            { question: "Хранение файлов онлайн?", "облако" },
            { question: "Сколько будет 5+5?", "10" },
            { question: "Сколько будет 12-4?", "8" },
            { question: "Сколько будет 6*6?", "36" },
            { question: "Сколько будет 81/9?", "9" },
            { question: "Число после 99?", "100" },
            { question: "Сколько сторон у треугольника?", "3" },
            { question: "Сколько граней у куба?", "6" },
            { question: "Сколько нулей в тысяче?", "3" },
            // Дополнительные вопросы из оригинального скрипта 1 (CS, Dota, Мемы и т.д.)
    ...
    HandleCasinoCommand(user, message) {
        ...
        // Пример базовой команды баланса из скрипта 5
