...
client.on('disconnected', (reason) => {
    console.log(`* Отключено от Twitch: ${reason}. Переподключение...`);
});
...
client.on('message', (target, context, message, self) => {
    ...
    if (text === "!фонд зп") {
        if (!hasRoleOrOwner(user)) { client.say(target, `❌ Нет прав.`); return; }
        client.say(target, `💼 Фонд зарплаты: ${salaryBank} КРЫШКИ.`);
        return;
    }
    ...
    if (text === "!казсчёт") {
        if (!hasRoleOrOwner(user)) { client.say(target, `❌ Нет прав.`); return; }
        client.say(target, `🏦 Банк казино: ${casinoBank} КРЫШКИ.`);
        return;
    }

    if (text === "!шопбанк") {
        if (!hasRoleOrOwner(user)) { client.say(target, `❌ Нет прав.`); return; }
        client.say(target, `🛒 Банк магазина: ${shopBank} КРЫШКИ.`);
        return;
    }
    ...
    if (text === "!шопбаланс" || text === "!мойшоп" || text === "!моитовары") {
        ...
        client.say(target, `🛒 ${getDisplayName(user)}, баланс в магазине: ${shopBal} КРЫШКИ.`);
        return;
    }
