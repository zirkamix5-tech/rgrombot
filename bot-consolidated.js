...
class TwitchCasino {
    ...
    ConnectToTwitch() {
        try {
            ...
            this.twitchClient.on('data', (data) => {
                ...
                this.readBuffer = lines.pop(); // keep unfinished line
    ...
    Command(user, message) {
        ...
        // НОВЫЕ КОМАНДЫ: Вывод крышек из казино обратно в нал или на карту
        ...
        if (text === "!моя роль") {
            if (!this.owners.includes(user)) { 
                this.Send("❌ Ты не сотрудник казино."); 
                return; 
            }
        ...
        if (text === "!меню" || text === "!админ") {
            if (!this.owners.includes(user)) { 
                this.Send("❌ Доступно только сотрудникам."); 
                return; 
            }
        ...
        if (text.startsWith("!стат") || text.startsWith("!статистика")) {
            if (!this.HasRoleOrOwner(user)) { 
                this.Send("❌ Доступно сотрудникам казино."); 
                return; 
            }
            ...
            this.Send("📊 СТАТИСТИКА [" + this.GetDisplayName(targetUser) + "] ➡️ Карта: " + balance + " | Наличные: " + targetProfile.balance + " | Фишки: " + targetProfile.casinoChips + " | Статус карты: " + cardSt + " | Долг: " + (this.debtAmount[targetUser] ? this.debtAmount[targetUser] : 0));
            return;
        }
        ...
        if (text.startsWith("!передать") || text.startsWith("!дать")) {
            ...
            if (targetUser === user) { 
                this.Send("❌ Нельзя переводить себе!"); 
                return; 
            }
            if (this.coins[user] < giveAmount) { 
                this.Send("❌ Недостаточно средств на карте!"); 
                return; 
            }
        ...
        if (text.startsWith("!долг")) {
            ...
            let currentDebt = this.debtAmount[user] ? this.debtAmount[user] : 0;
            if (currentDebt > 0) { 
                this.Send("❌ У тебя уже есть активный долг: " + currentDebt); 
                return; 
            }
        ...
        if (text.startsWith("!вернуть долг")) {
            ...
            let currentDebt = this.debtAmount[user] ? this.debtAmount[user] : 0;
            if (currentDebt <= 0) { 
                this.Send("ℹ️ У тебя нет долгов."); 
                return; 
            }
        ...
        if (text.startsWith("!снять долг")) {
            ...
            let parts = text.split(' ');
            if (parts.length < 3) { 
                this.Send("❌ Используй: !снять долг [ник]"); 
                return; 
            }
        ...
        if (text.startsWith("!снять каз")) {
            if (user !== "qumosx") { 
                this.Send("❌ Только Главный Босс может снимать деньги с банка Казино."); 
                return; 
            }
            let parts = text.split(' ');
            ...
            if (this.casinoBank < amount) { 
                this.Send("❌ Не хватает средств в казне."); 
                return; 
            }
        ...
        if (text.startsWith("!снять шоп")) {
            ...
            if (this.shopBank < amount) { 
                this.Send("❌ Недостаточно средств."); 
                return; 
            }
        ...
        if (text.startsWith("!чек")) {
            ...
            if (arg === "вип") { 
                this.Send("💎 ВИП (10k): Статус элитного игрока."); 
                return; 
            }
            if (arg === "удача") { 
                this.Send("🍀 Удача (15k): Выше шанс выигрыша."); 
                return; 
            }
