...
class UserProfile {
    constructor(username) {
        ...
        this.bankCardBalance = 0; // Банковская карта
...
class TwitchCasino {
    ...
    Start() {
        ...
        this.autoTimer = setInterval(() => this.CheckAutoCasinoTime(), 60000);
    }
    ...
    ConnectToTwitch() {
        try {
            ...
            this.twitchClient.on('data', (data) => {
                ...
                this.readBuffer = lines.pop(); // Оставляем неполную строку в буфере
    ...
    GetProfile(user) {
        if (!this.userProfiles[user]) {
            this.userProfiles[user] = new UserProfile(user);
        }
        
        this.CheckAndApplyHouseTax(this.userProfiles[user]);
        return this.userProfiles[user];
    }

    TryPayChoice(user, profile, cost, choice, callback) {
        let payMethodTag = "";
        let failReason = "";
        ...
        if (choice === "карта") {
            if (profile.isDebtCardBlocked) {
                failReason = "Ваша банковская карта заблокирована! Ей нельзя оплачивать, используйте наличные (!оплата нал).";
                return callback(false, payMethodTag, failReason);
            }
            if (userCoins < cost) {
                failReason = "Недостаточно средств на банковской карте! Нужно: " + cost;
                return callback(false, payMethodTag, failReason);
            }

            this.coins[user] = userCoins - Number(cost);
            profile.bankCardBalance = this.coins[user];
            payMethodTag = "!оплата карта";
            return callback(true, payMethodTag, failReason);
        } else if (choice === "нал") {
            if (profile.balance < cost) {
                failReason = "Недостаточно наличных средств! Нужно: " + cost;
                return callback(false, payMethodTag, failReason);
            }

            profile.balance -= Number(cost);
            payMethodTag = "!оплата нал";
            return callback(true, payMethodTag, failReason);
        }

        failReason = "Неверный способ оплаты. Используйте '!оплата карта' или '!оплата нал'.";
        return callback(false, payMethodTag, failReason);
    }

    TryPay(user, profile, cost, callback) {
        let payMethodTag = "";
        let userCoins = this.coins[user] !== undefined ? this.coins[user] : 0;
        ...
        if (!canUseCard && !canUseCash) {
            return callback(false, payMethodTag);
        }
        ...
        if (canUseCard) {
            ...
            profile.bankCardBalance = this.coins[user];
            payMethodTag = "!оплата карта";
            return callback(true, payMethodTag);
        }
    ...
    CheckAndApplyHouseTax(profile) {
        ...
        let daysPassed = Math.floor(diffTime / (1000 * 3600 * 24));
    ...
    Command(user, message) {
        ...
        if (lowerText.startsWith("!купить дом ")) {
            ...
            this.TryPayChoice(user, profile, cost, choice, (success, payMethod, failReason) => {
                if (!success) {
                    this.Send("❌ " + failReason);
                    return;
                }
                ...
                this.Send("🏡 (" + payMethod + ") " + this.GetDisplayName(user) + " успешно приобрел дом класса '" + houseArg + "'!");
            });
            return;
        }
        ...
        if (lowerText.startsWith("!оплатить налог") || lowerText.startsWith("!коммуналка") || lowerText.startsWith("!налог")) {
            ...
            this.TryPayChoice(user, profile, debt, choice, (success, payMethod, failReason) => {
                if (!success) {
                    this.Send("❌ " + failReason + " (Нужно: " + debt + ")");
                    return;
                }
                ...
                this.Send("💡 (" + payMethod + ") " + this.GetDisplayName(user) + " успешно оплатил коммуналку на сумму " + debt + " деняг!");
            });
            return;
        }
        ...
        if (lowerText === "!трудиться" || lowerText === "!смена") {
            ...
            let earned = 0;
            if (this.fixedJobsSalary[profile.job] !== undefined) earned = this.fixedJobsSalary[profile.job];
        ...
        if (lowerText.startsWith("!пополнить карту ") || lowerText.startsWith("!пополнить ")) {
            ...
            this.coins[user] += amount;
            profile.bankCardBalance = this.coins[user];
            this.SaveData();
            this.Send("💳 " + this.GetDisplayName(user) + " успешно пополнил банковскую карту на " + amount + " деняг! Баланс карты: " + this.coins[user]);
            return;
        }
        ...
        if (lowerText.startsWith("!обналичить ")) {
            ...
            if (source === "нал") {
                ...
                let cardBal = this.coins[user] !== undefined ? this.coins[user] : 0;
                if (cardBal < amount) {
                ...
                profile.casinoChips += amount;
                this.SaveData();
                this.Send("💳 " + this.GetDisplayName(user) + " купил " + amount + " фишек 👑 с банковской карты! Баланс карты: " + this.coins[user]);
                return;
            } else {
        ...
        if (lowerText.startsWith("!вывод ")) {
            ...
            if (targetDest === "нал") {
                ...
                profile.bankCardBalance = this.coins[user];
                this.SaveData();
                this.Send("💳 " + this.GetDisplayName(user) + " вывел " + amount + " фишек 👑 на банковскую карту! Баланс карты: " + this.coins[user]);
                return;
            } else {
        ...
        if (text.startsWith("!стат") || text.startsWith("!статистика")) {
            ...
            if (this.coins[targetUser] === undefined) {
                this.Send("❌ Игрок @" + targetUser + " не найден.");
                return;
            }

            let balance = this.coins[targetUser];
            let targetProfile = this.GetProfile(targetUser);
            let cardSt = targetProfile.isDebtCardBlocked ? "🔴 Заблокирована" : "🟢 Активна";
            let dAmount = this.debtAmount[targetUser] !== undefined ? this.debtAmount[targetUser] : 0;

            this.Send("📊 СТАТИСТИКА [" + this.GetDisplayName(targetUser) + "] ➡️ Карта: " + balance + " | Наличные: " + targetProfile.balance + " | Фишки: " + targetProfile.casinoChips + " | Статус карты: " + cardSt + " | Долг: " + dAmount);
            return;
        }
        ...
        if (text.startsWith("!передать") || text.startsWith("!дать")) {
            ...
            if (this.coins[user] < giveAmount) { this.Send("❌ Недостаточно средств на карте!"); return; }

            if (this.coins[targetUser] === undefined) { 
                this.coins[targetUser] = this.startCoins; 
                this.shopMoney[targetUser] = this.startCoins; 
            }

            this.coins[user] -= giveAmount;
            profile.bankCardBalance = this.coins[user];
            this.coins[targetUser] += giveAmount;
            let targetProf = this.GetProfile(targetUser);
            targetProf.bankCardBalance = this.coins[targetUser];
            this.SaveData();
        ...
        if (text.startsWith("!долг")) {
            ...
            let currentDebt = this.debtAmount[user] !== undefined ? this.debtAmount[user] : 0;
            if (currentDebt > 0) { this.Send("❌ У тебя уже есть активный долг: " + currentDebt); return; }

            this.debtAmount[user] = debtSum;
            this.debtTime[user] = new Date();
            this.debtBlocked[user] = false;
            this.coins[user] += debtSum; 
            profile.bankCardBalance = this.coins[user];
            this.SaveData();
        ...
        if (text.startsWith("!вернуть долг")) {
            ...
            this.TryPayChoice(user, profile, currentDebt, choice, (success, payMethod, failReason) => {
                if (!success) {
                    this.Send("❌ " + failReason + " (Нужно для возврата: " + currentDebt + ")");
                    return;
                }
                ...
                this.SaveData();
                this.Send("✅ (" + payMethod + ") " + this.GetDisplayName(user) + " полностью погасил долг! Карта разблокирована.");
            });
            return;
        }
        ...
        if (text.startsWith("!снять долг")) {
            ...
            this.debtAmount[targetUser] = 0;
            this.debtBlocked[targetUser] = false;
        ...
        if (text === "!зарплата" || text === "!зп") {
            ...
            for (let i = 0; i < this.owners.length; i++) {
                ...
                if (this.coins[staff] === undefined) {
                    this.coins[staff] = this.startCoins;
                    this.shopMoney[staff] = this.startCoins;
                }
                let personalShare = share + (i === 0 ? remainder : 0);
                this.coins[staff] += personalShare;
                this.GetProfile(staff).bankCardBalance = this.coins[staff];
            }
        ...
        if (text === "!*100#" || text === "*100#") {
            this.Send("💰 " + this.GetDisplayName(user) + " | Фишки казино: " + profile.casinoChips + " 👑 | Наличные: " + profile.balance + " | Карта: " + this.coins[user]);
            return;
        }

        if (text === "!шопбаланс" || text === "!мойшоп" || text === "!моитовары") {
            let shopBal = this.shopMoney[user] !== undefined ? this.shopMoney[user] : 0;
            this.Send("🛒 " + this.GetDisplayName(user) + ", баланс в магазине: " + shopBal + " крышек.");
            return;
        }

        if (text === "!топказ") {
            let topPlayers = Object.entries(this.userProfiles)
                .sort((a, b) => b[1].casinoChips - a[1].casinoChips)
                .slice(0, 5);
            let topList = [];
            for (let i = 0; i < topPlayers.length; i++) {
                let playerName = topPlayers[i][0];
                let displayN = this.customNicknames[playerName] ? this.customNicknames[playerName] : playerName;
        ...
        if (text.startsWith("!купить ")) {
            ...
            if (itemLower === "вип") { price = 10000; itemName = "VIP"; targetDict = this.vipBonus; }
                ...
                if (newNick !== "") {
                    ...
                    this.shopMoney[user] -= price;
                    this.shopBank += Math.floor(price / 2);
                    this.customNicknames[user] = newNick;
                    this.SaveData();
        ...
        if (text.startsWith("!каз")) {
            ...
            if (win > 0) {
                ...
                if (this.shopMoney[user] === undefined) this.shopMoney[user] = this.startCoins;
                this.shopMoney[user] += shopBonusIncome;
    ...
    CheckDebtStatus(user, profile) {
        if (this.debtAmount[user] !== undefined && this.debtAmount[user] > 0) {
            let spanHours = (new Date() - new Date(this.debtTime[user])) / (1000 * 60 * 60);
            ...
            if (spanHours >= 72) {
                ...
                if (spanHours >= 120) {
                    ...
                    if (userCoins < currentDebt && profile.balance < currentDebt) {
                        ...
                        profile.prisonReleaseTime = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().slice(0, 19).replace('T', ' ');
            ...
            } else if (this.debtBlocked[user] !== undefined && this.debtBlocked[user]) {
                ...
                for (let line of lines) {
                    ...
                    if (parts.length >= 2) {
                        if (parts[0] === "COIN" && parts.length >= 3) {
                            let val = parseInt(parts[2], 10);
                            this.coins[parts[1]] = val;
                            this.GetProfile(parts[1]).bankCardBalance = val;
                        } else if (parts[0] === "SHOP" && parts.length >= 3) {
                            this.shopMoney[parts[1]] = parseInt(parts[2], 10);
                        } else if (parts[0] === "CBANK" && parts.length >= 2) {
                            this.casinoBank = parseInt(parts[1], 10);
                        } else if (parts[0] === "SBANK" && parts.length >= 2) {
                            this.shopBank = parseInt(parts[1], 10);
                        } else if (parts[0] === "SALARY" && parts.length >= 2) {
                            this.salaryBank = parseInt(parts[1], 10);
                            ...
                            this.GetProfile(parts[1]).isDebtCardBlocked = (parts[2] === "True" || parts[2] === "true");
                        } else if (parts[0] === "PRISON" && parts.length >= 3) {
                            this.GetProfile(parts[1]).isImprisoned = (parts[2] === "True" || parts[2] === "true");
    ...
    SaveData() {
        try {
            ...
            for (let [key, value] of Object.entries(this.coins)) {
                lines.push("COIN:" + key + ":" + value);
            }
            for (let [key, value] of Object.entries(this.shopMoney)) {
                lines.push("SHOP:" + key + ":" + value);
            }
            for (let [key, value] of Object.entries(this.customNicknames)) {
                lines.push("NICK:" + key + ":" + value);
            }
            for (let [key, value] of Object.entries(this.userProfiles)) {
                lines.push("JOB:" + key + ":" + value.job);
                lines.push("MONEY:" + key + ":" + value.balance);
                lines.push("CHIPS:" + key + ":" + value.casinoChips);
                if (value.lastWorkDate && value.lastWorkDate !== "") {
                    lines.push("WORKDATE:" + key + ":" + value.lastWorkDate);
                }
                if (value.houseType && value.houseType !== "") {
                    lines.push("HOUSETYPE:" + key + ":" + value.houseType);
                    lines.push("HOUSEDEDB:" + key + ":" + value.houseTaxDebt);
                    lines.push("HOUSETDATE:" + key + ":" + value.lastTaxDate);
                }
                if (value.isDebtCardBlocked) {
                    lines.push("CARDLOCK:" + key + ":" + value.isDebtCardBlocked);
                }
                if (value.isImprisoned) {
                    lines.push("PRISON:" + key + ":" + value.isImprisoned);
                    lines.push("PRISONT:" + key + ":" + value.prisonReleaseTime);
                }
            }
