...
// ==========================================
// КЛАССЫ И СТРУКТУРЫ ДАННЫХ (Casino/Economy)
// ==========================================
class UserProfile {
...
class UnifiedTwitchBot {
    constructor() {
        // Настройки подключения к Twitch
        this.botName = process.env.BOT_NAME || "RGROMBOT";
        this.oauth = process.env.TWITCH_OAUTH || "oauth:4z39cw1q1hwfxhjmqwoha1hhr2zo52";
        this.channelName = process.env.MAIN_CHANNEL || "QumosX";

        // Каналы для автоприветствий
        this.channelsToJoin = process.env.CHANNELS 
            ? process.env.CHANNELS.split(',').map(c => c.trim()) 
            : ["Blind_Mdfk", "vctivanova", "QumosX", "r0magr0m"];
    ...
    start() {
        console.log("🚀 Запуск Unified Twitch Bot...");
        ...
        // HTTP сервер для Render.com health checks
    ...
    StartHealthServer() {
        ...
        server.listen(PORT, () => {
            console.log(`✅ Health check server запущен на порту ${PORT}`);
        });
    }
        ...
        try {
            ...
            this.client.connect(6667, 'irc.chat.twitch.tv', () => {
                ...
                channelsToJoinSet.forEach(ch => {
                    ...
                    console.log("📺 Присоединился к каналу: " + ch);
                });
            });
            ...
            this.client.on('close', () => {
                console.log("❌ Соединение закрыто. Переподключение через 5 сек...");
    ...
    StartQuiz(channel) {
        ...
        if (this.usedQuestions.length >= this.questions.length) {
            this.usedQuestions = [];
            this.Send(channel, "🔄 Все вопросы пройдены! Начинаем новый круг.");
        }
    ...
    HandleFamilyCommand(user, message) {
        ...
        if (lowerText === "!согласен" || lowerText === "!да") {
            ...
            this.SaveDataFamily();
            this.Send(this.channelName, "💒 @" + user + " и @" + partner + " поженились! 💕 Поздравляем молодоженов!");
            return;
        }
        ...
        if (lowerText === "!отказ" || lowerText === "!нет") {
            ...
            let partner = this.marriageProposals.get(user);
            this.marriageProposals.delete(user);
            this.Send(this.channelName, "💔 @" + user + " отказал(а) @" + partner + " в браке.");
            return;
        }

        // 4. Дети
        if (lowerText.startsWith("!ребенок")) {
            let parts = text.split(' ');
            if (parts.length < 3) {
                this.Send(this.channelName, "👶 @" + user + ", укажи двух родителей: !ребенок @родитель1 @родитель2 Имя");
                return;
            }
            let p1 = parts[1].replace("@", "").toLowerCase().trim();
            let p2 = parts[2].replace("@", "").toLowerCase().trim();
            let childName = parts.slice(3).join(" ");
            
            if (!childName || childName.length === 0) {
                this.Send(this.channelName, "👶 Укажи имя ребенка!");
                return;
            }

            this.children.set(childName.toLowerCase(), [p1, p2, childName]);
            this.SaveDataFamily();
            this.Send(this.channelName, "🎉 Поздравляем @" + p1 + " и @" + p2 + "! Родился ребенок " + childName + "! 👶");
            return;
        }
        ...
        if (lowerText.startsWith("!дети")) {
            ...
            let target = user;
            if (parts.length > 1) {
                target = parts[1].replace("@", "").toLowerCase().trim();
            }

            let userChildren = [];
            this.children.forEach(child => {
                if (child[0] === target || child[1] === target) {
                    userChildren.push(child[2]);
                }
            });
        ...
        // 6. Брак статус
        if (lowerText.startsWith("!брак_статус")) {
            let target = user;
            let parts = text.split(' ');
            if (parts.length > 1) {
                target = parts[1].replace("@", "").toLowerCase().trim();
            ...
            if (!this.marriages.has(target)) {
                this.Send(this.channelName, "💔 @" + target + " не состоит в браке.");
                return;
            }

            let partners = this.marriages.get(target);
            let statusText = "💒 @" + target + " состоит в браке с: ";
            
            for (let i = 0; i < partners.length; i++) {
                statusText += "@" + partners[i];
                if (i < partners.length - 1) statusText += ", ";
            }

            this.Send(this.channelName, statusText);
            return;
        }
...
process.on('SIGTERM', () => {
    console.log("📍 Бот остановлен сервером...");
    bot.OnApplicationQuit();
    process.exit(0);
});

console.log("✅ Twitch Bot запущен и готов к работе!");
